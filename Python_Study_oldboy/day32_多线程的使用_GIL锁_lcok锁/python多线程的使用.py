...
# 五、threading.Thread 中常用方法
        ...
        ## 3、jion(timeout) 阻塞主线程，默认主线程在将子线程start()后会直接继续执行主线程，但如果主线程需要在某子线程执行完毕后再执行，则可以在子线程start()后，调用子线程的join()方法
                ### timeout 不设置值表示主线程会一直等待，设置值时表示主线程只等待子线程执行设置的秒数后就会继续执行主线程
                    # def add(a,b):
                    #     thread = threading.current_thread()
                    #     print('result = %d' %(a+b))
                    #     for i in range(0,5):
